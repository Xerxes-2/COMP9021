...
class Polygons:
    class Polygon:
        ...
        def __init__(self, coord, found: np.ndarray, depth_map):
            ...
            self.yDim, self.xDim = found.shape
            self.new_found = found.copy()
            self.banned = set()
            self.circle = self.__dfs([], coord)
            if not self.circle:
                raise PolygonsError.error_builder().no_expected_polygon().build()
            for p in self.circle:
                self.new_found[p] = 1
            self.vertex = self.__vertex()
            self.nb_ver = len(self.vertex)
            self.perimeter = self.__perimeter()
            self.area = self.__area()
            self.convex = self.__convex()
            self.rotations = self.__rotations()

        def __vertex(self):
            ver = [self.start, self.circle[1]]
            for i, (y2, x2) in enumerate(self.circle[2:]):
                y0, x0 = ver[- 2]
                y1, x1 = ver[- 1]
        ...
        def __clock(self, coord, from_coord):
            ...
            next_list = [(y, x) for (y, x) in next_list if
                         -1 < y < self.yDim and -1 < x < self.xDim and self.depth == self.depth_map[y, x] and (
                             y, x) not in self.banned and not self.new_found[y, x]]
            return next_list

        def __dfs(self, circle: list, coord):
            if coord == self.start and circle:
                return circle
            if coord in circle:
                return []
            if not circle:
            ...
            for next_coord in clock_list:
                ...
                if result:
                    return result
            self.banned.add(coord)
            return []

        def __perimeter(self):
            result = [0, 0]
            for i, (y, x) in enumerate(self.circle):
                y0, x0 = self.circle[i - 1]
        ...
        def show(self):
            ...
            print(f'    Convex: ' + self.convex * 'yes' + (not self.convex) * 'no')
            print(f'    Nb of invariant rotations: {self.rotations}')
    ...
    def __init__(self, filename: str):
        ...
        shape = raw.shape
        depth = np.asarray([[99 for _ in row] for row in raw])
        current_depth = -1
        finished = 0
        while not finished:
    ...
    def display(self):
        ...
        with open(self.name + 'tex', 'w') as f:
            f.write(f'''\\documentclass[10pt]\u007Barticle\u007D
\\usepackage\u007Btikz\u007D
...
\\draw[ultra thick] (0, 0) -- ({self.xDim - 1}, 0) -- ({self.xDim - 1}, \
{self.yDim - 1}) -- (0, {self.yDim - 1}) -- cycle;
''')
